   	...
   	<div class="wrap">
		...
		<ul class="megamenu skyblue">
     	...
     	<div class="register_account">
          	<div class="wrap">
          	<h4 class="title">Sửa thông tin sản phẩm</h4><br>
	          		<form method="POST" action="<?php echo "update_sp.php?masanpham=$masp&manhacungcap=$mancc&maloaikhachhang=$malkh&maloaisanpham=$malsp&tensanpham=$tensp&size=$size&soluong=$soluong&dongia=$dongia" ?>">
	          			<div>
	    	     			<label>Nhà cung cấp	: </label><br>
		    	     		<select name="newnhacc" value="<?php echo $mancc; ?>" style="width: 400px;">
			    	     		<?php
				    	     		$results = $mysqli->query("SELECT * FROM ncc");
									while($obj = $results->fetch_object()) {
								?>
									<option <?php if($obj->MaNCC == $mancc){echo("selected");}?> ><?php echo $obj->MaNCC ?></option>
								<?php }?>
							</select>
						</div>
	    	     		<div>
	    	     			<label>Loại khách hàng	: </label><br>
		    	     		<select name="newloaikh" value="<?php echo $malkh; ?>" style="width: 400px;">
			    	     		<?php
				    	     		$results = $mysqli->query("SELECT * FROM Loaikh	");
									while($obj = $results->fetch_object()) {
								?>
									<option <?php if($obj->MaLKH == $malkh){echo("selected");}?> ><?php echo $obj->MaLKH ?></option>
								<?php }?>
							</select>
						</div>
	    	     		<div>
	    	     			<label>Mã loại sản phẩm: </label><br>
	    	     			<select name="newmaloaisp" value="<?php echo $malsp; ?>" style="width: 400px;">
			    	     		<?php
				    	     		$results = $mysqli->query("SELECT * FROM loaisp	");
									while($obj = $results->fetch_object()) {
								?>
									<option <?php if($obj->MaLoaiSp == $malsp){echo("selected");}?> ><?php echo $obj->MaLoaiSp ?></option>
								<?php }?>
							</select>
	    	     		</div>
	    	     		<div>
	    	     			<label>Tên sản Phẩm: </label>
	    	     			<input type="text" name="newtensp" class="form-control" style="width: 400px;" value="<?php echo $tensp ?>">
	    	     		</div>
		    	     	<div>
		    	     		<label>Size: </label>
		    	     		<input type="text" name="newsize" class="form-control" style="width: 400px;" value="<?php echo $size ?>">
		    	     	</div>
		    	     	<div>
		    	     		<label>Đơn giá: </label>
		    	     		<input type="int" name="newdongia" class="form-control" style="width: 400px;" value="<?php echo $dongia ?>">
		    	     	</div>
		    	     	<div>
		    	     		<label>Số lượng: </label>
		    	     		<input type="int" name="newsoluong" class="form-control" style="width: 400px;" value="<?php echo $soluong ?>">
		    	     	</div>
		    	     	<br>
