		...
		<ul class="megamenu skyblue">
     	...
     	<div class="register_account">
          	...
          	<h4 class="title">Thêm sự kiện</h4><br>
	          		<form method="POST" action="<?php echo "add_sk.php"?>">
	          			<div>
	    	     			<label>Mã sự kiện: </label>
	    	     			<input type="text"  name="newmasukien" class="form-control" style="width: 400px;">
	    	     		</div>
	          			<div>
	    	     			<label>Tên sự kiện: </label>
	    	     			<input type="text"  name="newtensukien" class="form-control" style="width: 400px;">
	    	     		</div>
	    	     		<div>
	    	     			<label>Ngày bắt đầu: </label><br>
		    	     		<input type="date" name="newngaybatdau" class="form-control" style="width: 400px;">
						</div>
	    	     		<div>
	    	     			<label>Ngày kết thúc: </label>
	    	     			<input type="date" name="newngayketthuc" class="form-control" style="width: 400px;">
	    	     		</div>
		    	     	<label>Mã sản phẩm: </label><br>
	    	     			<select name="newmasanpham" value="" style="width: 400px;">
			    	     		<?php
				    	     		$results = $mysqli->query("SELECT * FROM sanpham");
									while($obj = $results->fetch_object()) {
								?>
									<option><?php echo $obj->MaSp ?></option>
								<?php }?>
							</select>
