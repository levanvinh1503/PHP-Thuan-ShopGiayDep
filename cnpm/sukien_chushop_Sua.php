<?php
	...
	$mask=$_GET['masukien'];
	$tensk=$_GET['tensukien'];
	$ngaybd=$_GET['ngaybatdau'];
	$ngaykt=$_GET['ngayketthuc'];
	$masp=$_GET['masanpham'];
   	...
   	<div class="wrap">
		...
		<ul class="megamenu skyblue">
     	...
     	<div class="register_account">
          	<div class="wrap">
          	<h4 class="title">Sửa thông tin sự kiện</h4><br>
	          		<form method="POST" action="<?php echo "update_sk.php?masukien=$mask&tensukien=$tensk&ngaybatdau=$ngaybd&ngayketthuc=$ngaykt&masanpham=$masp" ?>">
	          			<div>
	    	     			<label>Tên sự kiện: </label>
	    	     			<input type="text" name="newtensk" class="form-control" style="width: 400px;" value="<?php echo $tensk ?>">
	    	     		</div>
	    	     		<div>
	    	     			<label>Ngày bắt đầu	: </label><br>
		    	     		<input type="date" name="newngaybatdau" class="form-control" style="width: 400px;" value="<?php echo $ngaybd ?>">
						</div>
	    	     		<div>
	    	     			<label>Ngày kết thúc: </label><br>
	    	     			<input type="date" name="newngayketthuc" class="form-control" style="width: 400px;" value="<?php echo $ngaykt ?>">
	    	     		</div>
	    	     		<div>
	    	     			<label>Mã sản phẩm: </label><br>
	    	     			<select name="newmasanpham" value="<?php echo $masp; ?>" style="width: 400px;">
			    	     		<?php
				    	     		$results = $mysqli->query("SELECT * FROM sanpham");
									while($obj = $results->fetch_object()) {
								?>
									<option <?php if($obj->MaSp == $masp){echo("selected");}?> ><?php echo $obj->MaSp ?></option>
								<?php }?>
							</select>
		    	     	</div>
