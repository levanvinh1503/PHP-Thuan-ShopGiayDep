...
<body>
  <div class="header-top">
   ...
   <div class="header-bottom">
       ...
       <div class="login">
         <div class="wrap">
       <h4 class="title"> Danh sách các sự kiện</h4>
       		<form class="form">
       			<table class="table">
       				<tr>
       					<th>Mã Sự Kiện</th>
       				...
       				<?php
       					...
       					$results = $mysqli->query("SELECT * FROM sukien inner join sanpham on sukien.MaSp=sanpham.MaSp ");
       					if ($results) { 
					        while($obj = $results->fetch_object())
					        {
					        	...
					        	echo '<tr>';
					        		echo '<td>'.$obj->MaSK.'</td>';
					        		echo '<td>'.$obj->TenSK.'</td>';
					        		echo '<td>'.$obj->NgayBD.'</td>';
					        		echo '<td>'.$obj->NgayKT.'</td>';	
					        		echo '<td>'.$obj->TenSanPham.'</td>';
					        		echo '<td><a href="sukien_chushop_Xoa.php?masukien='.$obj->MaSK.'">xóa</a></td>';
					        		echo '<td><a href="sukien_chushop_Sua.php?masukien='.$obj->MaSK.'&tensukien='.$obj->TenSK.'&ngaybatdau='.$obj->NgayBD.'&ngayketthuc='.$obj->NgayKT.'&masanpham='.$obj->MaSp.'">Sửa</a></td>';
					        	echo '</tr>';
       		...
       		<a href="sukien_chushop_Them.php"><button class="btn btn-info">Thêm sự kiện</button></a>
       		</div>
       		</div>
	    <div class="footer">
